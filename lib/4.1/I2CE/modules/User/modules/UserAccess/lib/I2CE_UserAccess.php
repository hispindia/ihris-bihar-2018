...
class I2CE_UserAccess extends I2CE_UserAccess_Mechanism {
    ...
    public function _userHasDefaultPassword($username) {
      ...
      */
    public function logActivity($username, $activity='access', $ip_address='127.0.0.1', $timestamp= null) {
        if ($timestamp === null) {
            $timestamp = time();
        }
        ...
        if ( !$check && !I2CE::pearError( $check, "Error finding entry in user_log table: ", E_USER_WARNING ) ) {
          I2CE::raiseError("Logging you in the first part with $ip_address");
            $res = $this->db->execParam(
                    "INSERT INTO " . $this->logTable . " (user, login, session_id, activity, ip_address ) VALUES 
                    ( IFNULL((SELECT id FROM " . $this->detailTable . " WHERE username = ?),0), NOW(), ?, NOW(), ? )",
                    array( $username, session_id(),$ip_address), array( 'text', 'text', 'text' ) );
        ...
        case 'access':
            $res = $this->db->execParam(
                    "UPDATE " . $this->logTable . " SET activity = NOW(), ip_address = ? WHERE user = IFNULL((SELECT id FROM " . $this->detailTable . " WHERE username = ?),0) AND session_id = ? AND logout IS NULL",
                    array( $ip_address, $username, session_id() ), array( 'text','text', 'text' ) );
    ...
    /**
      ...
      *  @param string $username
      * @param string $ip_address defaults to 127.0.0.1
      * @returns boolean. true when a user is attempting multiple logins
      */
    public function isLoggingFromAnotherIP($username, $ip_address){
        $checkIP = false;
        I2CE::getConfig()->setIfIsSet( $checkIP, "/config/site/check_login_ip" );
        if($checkIP && $username != 'i2ce_admin'){
            $userDetails = $this->db->getRow( "SELECT user, ip_address FROM " . $this->logTable 
                    . " WHERE user = IFNULL((SELECT id FROM " . $this->detailTable . " WHERE username = ?),0) AND logout IS NULL ORDER BY login DESC", null, array( $username ), array( 'text' ) );
            if(count($userDetails) != 0){
                if($userDetails->ip_address != $ip_address && $userDetails->user != 0){
                    I2CE::raiseError("$username (from $ip_address) is already logged in from ".$userDetails->ip_address);
                    return true;
                }
            }
        }
