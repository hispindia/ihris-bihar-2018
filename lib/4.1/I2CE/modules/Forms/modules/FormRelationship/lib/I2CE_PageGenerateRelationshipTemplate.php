...
class I2CE_PageGenerateRelationshipTemplate extends I2CE_PageActionRelationship {
    ...
    protected function setStyleVars($odf,$data = null) {
        ...
        $name = $user->firstname . ' ' . $user->lastname;
        try {
            $odf->setStyleVars( '++user_name', $name, $this->encoding, $this->charset );
        } catch ( OdfException $e ) {
            //It's ok if it's not there so don't do anything.
        }
        $time = strftime("%c");
        try {
            $odf->setStyleVars( '++time', $time, $this->encoding, $this->charset );
        } catch ( OdfException $e ) {
            //It's ok if it's not there so don't do anything.
        }
        if ($data == null) {
    ...
    protected function outputTemplate($odf,$basename,$saveOnFormObj=false,$fieldName=false,$saveFileOnDB=false) {
        ...
        if ($unoconv 
            ...
            if ($error == 0) {
                ...
                //save a copy of the document into formfield DOCUMENT
                if ($saveFileOnDB){
					if( !($saveOnFormObj instanceof I2CE_Form)){
						I2CE::raiseError("Parameter saveFileOnDB is true but FormObject is invalid");
					}else{
						if($fieldName != ""){
							$fp = fopen($conv_temp_file, 'r');
							$fread = fread($fp, filesize($conv_temp_file));
							$getfile = file_get_contents($conv_temp_file, true);
							$saveOnFormObj->populate();
							$fieldObj=$saveOnFormObj->getField($fieldName);
							if( !($fieldObj instanceof I2CE_FormField_DOCUMENT)){
								I2CE::raiseError("Parameter saveFileOnDB is true but FieldObject is invalid");
							}else{
								$saveOnFormObj->getField($fieldName)->setFromData($getfile, $basename, "application/pdf");
								$user = new I2CE_User();
								$saveOnFormObj->save($user);
								$saveOnFormObj->cleanup();
							}
						}
					}
				}
