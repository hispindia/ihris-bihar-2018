...
abstract class I2CE_CustomReport_Display extends I2CE_Fuzzy{
    ...
    protected function mapResults($result) {
        ...
        foreach ($result as $reportformfield => $value) {
            //change for data hiding by ifhaam on 6-1-2018 starts
            $role = I2CE_UserAccess_Mechanism::getSessionRole();

            if($role!='admin') {

                if ($reportformfield == "work+mobile_phone" && strlen($value)>0) {
                    $stars = "******************************";//size 30
                    $len = strlen($value);
                    $value = substr($stars,34-$len).substr($value,$len-4);
                }
                if($reportformfield=="person_position_salarybreakdown+bankaccnum" && strlen($value)>0){
                    $stars = "******************************";//size 30
                    $len = strlen($value);
                    $value = substr($stars,34-$len).substr($value,$len-4);
                }
                if($reportformfield=="person_position_salarybreakdown+pannumber" && strlen($value)>0){
                    $stars = "******************************";//size 30
                    $len = strlen($value);
                    $value = substr($stars,34-$len).substr($value,$len-4);
                }
            ...
            //TODO:add the fields which should be hiden in report view
    ...
    protected function getMappedValue($reportformfield,$value,$isReportForm=true) {
        ...
        if ( $a_aggregate ) {
            // Do nothing since it's an aggregate field and a mapped lookup would be weird.
            return $value;
        }
        if ( !$formfield ) {
