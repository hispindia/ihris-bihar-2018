...
class I2CE_Date {
    ...
    private static $types = array( self::YEAR_ONLY => "Year", self::MONTH_DAY => "Month and Day",
                                   self::DATE => "Date", self::DATE_TIME => "Date and Time", self::TIME_ONLY => "Time" );
    ...
    public static function now( $type = self::DATE_TIME, $time_stamp = null, $strict = false ) {
        if (is_array($time_stamp)) {
            if ( array_key_exists( 'value', $time_stamp ) ) {
                $time_stamp = I2CE_Date::fromDB( $time_stamp['value'] )->getValues();
            }
            $now = $time_stamp;
        ...
        case self::DATE_TIME :
            ...
            if (!array_key_exists('mday',$now) || !$now['mday'] 
                ...
                || !array_key_exists('year',$now)  || !$now['year'] 
                || !array_key_exists('seconds',$now) || !$now['seconds'] 
                || !array_key_exists('minutes',$now) || !$now['minutes'] 
                || !array_key_exists('hours',$now) || !$now['hours']  ) {
                if ($strict === 'blank') {
                    return self::blank($type);
                } else if ($strict) {
                    return false;
        ...
        case self::TIME_ONLY :
            ...
            if (!array_key_exists('seconds',$now) || !$now['seconds'] 
                || !array_key_exists('minutes',$now) || !$now['minutes'] 
                || !array_key_exists('hours',$now) || !$now['hours']  ) {
                if ($strict === 'blank') {
                    return self::blank($type);
    ...
    public static function fromDB( $dateString, $type = null ) {
        ...
        if ( preg_match( '/^(\d+)-(\d+)-(\d+)( 00:00:00){0,1}$/', $dateString, $data ) ) {            
            if ( $type === self::MONTH_DAY || $data[1] == '0000' ) {
                $date = new I2CE_Date( self::MONTH_DAY );
                $date->month = $data[2];
                $date->day = $data[3];
