...
if (!class_exists('Spreadsheet_Excel_Writer')) {
    usage('Cannot find Spreadsheet_Excel_Writer: pear install --alldeps -f Spreadsheet_Excel_Writer ');
}
...
foreach ($locales as $locale) {    
    ...
    if (I2CE::pearError($workbook,"Could not create workbook")) {
        continue;
    }
    ...
    foreach ($pot_array as $module =>$pot) {
        ...
        if (I2CE::pearError($worksheet,"Can't add worksheet: $ws")) {
            continue;
        }
        $worksheet->setColumn(0,1,75);
        ...
        foreach (array_keys($pot) as $msgstr) {
            ...
            $worksheet->write($row,0,$msgstr);
            if ($trans = translate($msgstr)) {
                $worksheet->write($row,1,$trans);
            }
            $row++;
