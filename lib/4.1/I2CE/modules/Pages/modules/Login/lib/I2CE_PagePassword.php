    ...
    protected function action() {
        ...
        if ( $this->isPost() ) {
            ...
            if ( !$access instanceof I2CE_UserAccess_Mechanism || !$access->canChangePassword() || $this->user->getRole() == 'guest' ) {
                ...
                $success = 'no_match';
            } else if ( isset($this->args['length']) && strlen($this->post('confirm_password') ) < $this->args['length'] ) {
                $success = 'short';
            } else if ( isset($this->args['allow_all_characters']) && 
                              $this->args['allow_all_characters'] == 1  ) {
                              if (!I2CE_Validate::checkPassword( $this->post('confirm_password') ) ) {
                $success = 'weak';
              }
            ...
            if ( $success == 'success') {
                ...
                if($success == 'short'){
                  $lengthNode = $this->template->getElementById('pass_length');
                  $textNode = $this->template->createElement('span','', "The password must be at least ".$this->args['length']." characters long. Please try again.");
                  $this->template->appendNode($textNode,$lengthNode);
                }
            }
        } else {
            $this->template->addFile( "password_form.html" );
