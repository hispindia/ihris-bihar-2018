...
class I2CE_Updater {
    ...
    protected static function install($site_module_file,$restart, $verbose_errors =true ) {
        ...
        if (I2CE::hasWarnings()) {
            I2CE::raiseError( "Your site was not succesfully initialized. Please refer to the warning messages above.");                                
            flush();
            return false;
        } else {
            if (array_key_exists('HTTP_HOST',$_SERVER)) {
                $url = array_key_exists('REQUEST_URI', $_SERVER) ? $_SERVER['REQUEST_URI'] : "/";
                I2CE::raiseError("Congratulations, your site has been succesfully initialized. You may <a href='$url'>continue</a> onto the site.");
    ...
    protected static function _updateModules($updates,$removals=array(), $optional_excludes = array(), $disables = array() ) {
        ...
        while (count($to_enable) > 0) {
            ...
            //keep the old version set around until we know that the module upgraded succesfully
            ...
            I2CE::raiseError("Updating module config hash for $shortname");
            $mod_factory->setModuleHash($shortname); 
            I2CE::raiseError("Updating module class hash for $shortname");
            ...
            //we succesfully updated this module.  update the permanent modules config data with the temporary
            $mod_factory->loadPaths($shortname,null,true); //for the loading of all categories for this module
        }        
