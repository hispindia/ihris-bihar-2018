...
class I2CE_CustomReport_Display_Selector extends I2CE_CustomReport_Display_Default{
    ...
    protected function displayReportControls($contentNode, $controls=null) {
        ...
        $action .= '?' . implode("&", $flds);
        $this->template->addAjaxUpdate($this->getReportPrefix() . 'report_results_with_limits',$this->getReportPrefix().'Selector_submit','click',$action,$this->getReportPrefix().'report_results_with_limits',array('stub_events','formworm','treeselect'),$limit_form,true);
        return true;
    }
