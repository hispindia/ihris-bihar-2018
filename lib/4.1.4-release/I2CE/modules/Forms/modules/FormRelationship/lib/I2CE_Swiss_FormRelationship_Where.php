...
class I2CE_Swiss_FormRelationship_Where extends I2CE_Swiss_FormRelationship_Base {
    ...
    public function displayValues($contentNode,$transient_options, $action) {
        $mainNode = $this->template->appendFileByNode('formRelationship_where_container.html','div',$contentNode);
    ...
    protected function processFieldLimit($vals) {
        ...
        if (!$formObj instanceof I2CE_Form) {
            I2CE::raiseError("Invalid form at " . $this->factory->getPath($formPathComponents));
            return false;
        }
        ...
        $reportformfield = $this->getReportForm()  . '+' . $fieldObj->getName();
        $data = null;
    ...
    protected function displayWhereChoice( $contentNode, $action) {
        ...
        $mainNode = $this->template->appendFileByNode('formRelationship_new_limit_choice.html','div',$contentNode);        
    ...
    protected function displayExistingLimit($contentNode, $action ){
        ...
        $mainNode = $this->template->appendFileByNode('formRelationship_existing_limit.html','div',$contentNode);        
        if (!$mainNode instanceof DOMNode) {
        ...
        $node = $fieldObj->$method($this->template,'',$data);
        ...
        $ls = $this->renameInputs('*',$node,'limits:'.$field.':'.$style,'', true); //prefix all inputs with "swissFactoty:values:path:in:swiss:facotyr:limits:$field:$style:"
        return true;
    }
    ...
    protected function getJoinField() {
        $swissForm = $this->getAncestorByClass('I2CE_Swiss_FormRelationship');
        if (!$swissForm instanceof I2CE_Swiss_FormRelationship) {
            I2CE::raiseError("Could not get parent form");
            return null;
        }
        $path = 'join_data/field';
        if (!$swissForm->getStorage()->is_scalar($path)) {
            return null;
        }
        return $swissForm->getStorage()->$path;
    }
    ...
    protected function getReportForm() {
        ...
        return $swissForm->getStorage()->getName();
    }

    protected function isRelationshipBase() {
        $swissRelationship = $this->getRelationship();
    ...
    protected function displayLimitMenu( $contentNode, $action) {
        ...
        $form =  $this->getForm();
        if (!$form) {
            I2CE::raiseError("Could not get form name at " . $this->factory->getPath($formPathComponents));
            return false;
        }
        $factory = I2CE_FormFactory::instance();
        $formObj = $factory->createContainer($form);
        ...
        $mainNode = $this->template->appendFileByNode('formRelationship_new_limits.html','div',$appendNode);
        if (!$mainNode instanceof DOMNode) {
        ...
        $excludes =   I2CE::getConfig()->modules->CustomReports->limit_excludes->relationship_form->getAsArray();
        if ($this->isRelationshipBase()) {
            $excludes = array_merge($excludes,I2CE::getConfig()->modules->CustomReports->limit_excludes->primary_form->getAsArray());
        }
        $linked_field = $this->getJoinField();        
        ...
        foreach ($fields as $field) {
            ...
            $limitStylesNode = $this->template->appendFileByNode('formRelationship_new_limit_styles.html','tr',$limitsNode);
	    ...
	    //from selectUpdate $(this.options.prefix + ':' + this.select_id + ':'+ val);
            ...
            foreach ($limitStyles as $style) {                
                $method = 'getLimitMenu_' . $style;
                $limitStyleNode = $fieldObj->$method($this->template,''); //don't need a prefix here as it is dealt with below in rename inputs
                if (!$limitStyleNode instanceof DOMNode){ 
                    I2CE::raiseError("Invalid data returned from getLimitMenu_$style for field $field");
                    continue;
                }
                $limitContainerNode = $this->template->appendFileByNode('formRelationship_new_limit_style.html','tr',$limitsNode);
                ...
                $ls = $this->renameInputs('*',$limitStyleNode,'limits:'.$field.':'.$style,'', true);
