...
class I2CE_CachedForm extends I2CE_Fuzzy{
    ...
    public function updateCachedTable( $id, $check_mod=false ) {
        ...
        if ($this->formMech instanceof I2CE_FormStorage_DB) {
            if (!$this->fastPopulate($check_mod, $id)) {
                return false;
            }
        } else {
            if (!$this->slowPopulate($check_mod, $id)) {
                return false;
            }
        }
        return true;
     ...
     */
    protected function fastPopulate($check_mod=true, $id=null) {
        $fields = array();
        ...
        $select = $this->formMech->getRequiredFieldsQuery($this->form,$sel_fields,$id,true, $callback, $mod_time);
        ...
        $select = "SELECT  concat('{$this->form}|',id) as id, parent , IFNULL(`last_modified`,'1900-01-01 00:00:00') as `last_modified`, IFNULL(`created`,'0000-00-00 00:00:00') as `created`, " . implode(',',$d_fields) . " FROM ($select) AS cached_table";
        ...
        $db = MDB2::singleton();
        if (I2CE::pearError($prp = $db->prepare( $insertQry,$type_fields, MDB2_PREPARE_MANIP ),"Could not prepare fast populate")) {
            return false;
        }
    ...
    protected function slowPopulate($check_mod= true, $id = null) {
        ...
        $list = I2CE_FormStorage::listFields($this->form,$fields,true, $where, array(), false,$mod_time);
        I2CE_FormStorage::releaseStorage($this->form);
        $count = 0;
