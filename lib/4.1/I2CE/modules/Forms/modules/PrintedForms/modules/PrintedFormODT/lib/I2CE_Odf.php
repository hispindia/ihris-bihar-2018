...
IMG;
    ...
    /**
	 ...
	 public function exportAsAttachedFile($name=""){
		 $modulefactory = I2CE_ModuleFactory::instance();
		 $tmp_dir = sys_get_temp_dir();
                 $no_unoconv = false;
		 if(!$no_unoconv && $modulefactory->isEnabled('PrintedFormsODT')){
			$this->_save();
			$unoconv = false;
			ob_start();
			...
			if ($unoconv 
			   ...
			   && is_string($ext = $conversion['ext'])
				...
				if ( $multiple ) {
					...
					$exec = "HOME=/tmp/unoconv_homes/home$home_idx $unoconv --port=400$home_idx -o $conv_temp_file $export_options -f $format $this->tmpfile";
				} else {
					$exec = $unoconv . ' -o ' . $conv_temp_file . $export_options .' -f ' . $format . ' ' .  $this->tmpfile;
				}

				//NEED TO DO ON UBUNTU SERVER: sudo mkdir /home/www-data; sudo  chown www-data:www-data /home/www-data
				$error = 0;
				I2CE::raiseError($exec);
				system ($exec,$error);
				if ( $multiple ) {
			 ...
			 parent::exportAsAttachedFile($name);
			}			 
         }		 
		 else{			 
			 parent::exportAsAttachedFile($name);
