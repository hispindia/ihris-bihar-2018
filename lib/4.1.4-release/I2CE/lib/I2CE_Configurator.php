        ...
        public function importLocalizedTemplates( $localized = array()) {
            ...
            foreach ($dirs as $dir) {
                foreach ($this->locales as $locale) {
                    ...
                    $node = $this->template->loadFile($localized_file,'I2CEConfiguration');
                    if (!$node instanceof DOMNODE) {
                        continue;
                    }
                    $results = $this->template->query('./configurationGroup', $node);
                    $localenode = null;
                    if ($results->length == 1) {
                    ...
                    $results= $this->template->query('./metadata/version',$node);
                    if ($results->length != 1) {
                    ...
                    if ($localenode) {
                        $configNode->appendChild($localenode);
                    }
                    $data = array(
