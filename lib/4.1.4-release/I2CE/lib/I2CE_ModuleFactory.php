...
class I2CE_ModuleFactory { 
    ...
    public function updateHooks($shortnames=null,$force =false,$checkAccessTime =true) {
        ...
        foreach ($shortnames as $shortname) {
            ...
            if ($checkAccessTime &&$this->isUpToDateModule($shortname)) {
                    I2CE::raiseError("Continuing on $shortname b/c module up to date");
                    continue;
            }
            ...
            foreach (array('hooks'=>'getHooks','CLI_hooks'=>'getCLIHooks') as $hookType=>$hookTypeMethod) {
                $hooks =  $class->$hookTypeMethod();
                foreach ($hooks as $hook=>$data) {
                    $priorities = array();
                    if (! (is_string($hook) && strlen($hook) > 0)) {
                        I2CE::raiseError("$hookTypeMethod() Invalid hook: $hook for $shortname");
                        continue;
                    }
                    if (!preg_match('/^' . $phpfunc . '$/',$hook)) {
                        I2CE::raiseError("$hookTypeMethod()  Invalid hook: $hook for $shortname is not a php function");
                        continue;
                    }
                    if (is_string($data)) {
                        $priority= 50;
                    ...
                    foreach ($priorities as $p=>$m) {
                        if (!(is_string($m) && strlen($m) >0)) {
                            I2CE::raiseError("$hookTypeMethod() Invalid method  " . print_r($m,true) . "  for $shortname");
                            continue;
                        }       
                        if (!preg_match("/^$phpfunc\$/",$m)) {
                            I2CE::raiseError("$hookTypeMethod()  Invalid hook " . print_r($m,true)  . " for $shortname is not a php function");
                            continue;
                        }
                        if (!( (is_string($p) && strlen($p) >0 && ctype_digits($p))  || is_int($p))) {
                            I2CE::raiseError("$hookTypeMethod()  Invalid priority " . print_r($p,true)  . " for $shortname");
                            continue;
                        }
                        $this->config->$hookType->$hook->$p->$shortname = $m;
                    }
                    $this->config->$hookType->$hook->ksort(SORT_NUMERIC);
                }
            }
    ...
    public function removeHooks($shortnames) {
        ...
        foreach (array('hooks','CLI_hooks') as $hookType) {
            $hooks = $this->config->traverse($hookType,false,false);
        ...
        foreach($shortnames as $shortname) {
            ...
            if ($module instanceof I2CE_Module) {
                ...
                if (!($module->action_disable()))  {
                    I2CE::raiseError("Warning: could not succesfully disable $shortname");
                }                            
            }
     ...
     * @param mixed $arg.  The argument to pass to the methods.  Defaults to null, meaning there is no argument.
     * @returns an array containing all the return values of each of the hooked methods.
     */
    public static function callHooks($hook,$arg = null) {
        $mod_factory = self::instance();
        return $mod_factory->_callHooks($hook,$arg);
    }

    /**
     * Method to actually call the hooks
     */
    protected function _callHooks($hook,$arg) {
        $ret = array();
        $methods = $this->getHooks($hook);
        foreach ($methods as $data) {
            $module=$this->getClass($data['module']);
            $method=$data['method'];
            if ($arg === null) {
                $ret[]=$module->{$method}();
            } else {
                $ret[]=$module->{$method}($arg);
            }
        }
        return $ret;
    }
    ...
    public function getHooks($hook, $only_enabled = true) {
        ...
        foreach ($hooks->$hook as $priority=>$shortnames) {
            ...
            foreach ($shortnames as $shortname=>$method) {
                ...
                if (!$module instanceof I2CE_Module) {
                    I2CE::raiseError("module $module for $shortname cannot be associated with a subclass of I2CE_Module");
                    continue;
                }
