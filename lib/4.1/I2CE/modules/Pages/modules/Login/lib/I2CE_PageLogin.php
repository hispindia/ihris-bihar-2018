...
class I2CE_PageLogin extends I2CE_Page {
    ...
    protected function getHome() {
        ...
        protected static function setupDB() {
            if ( !self::$db instanceof MDB2_Driver_Common ) {
                ...
                self::$prepared = array();
                self::$prepared['user_log'] = self::$db->prepare("SELECT login,logout,activity FROM user_log WHERE user = ? ORDER BY activity DESC LIMIT 2", 
                        array( 'integer' ), array( 'date', 'date', 'date' ) );
                if ( I2CE::pearError( self::$prepared['user_log'], "Error preparing user_log statement: " ) ) {
                    return false;
                }
                self::$prepared['user'] = self::$db->prepare("SELECT id FROM user WHERE username = ? LIMIT 1", 
                        array( 'text' ), array( 'integer' ) );
                if ( I2CE::pearError( self::$prepared['user'], "Error preparing user statement: " ) ) {
                    return false;
                }
            }
            return true;
    ...
    protected function action() {
        ...
        if ( $this->user->logged_in() ) {
            ...
            if ( I2CE_User::userHasDefaultPassword($this->user->username) ) {
                $this->userMessage(
