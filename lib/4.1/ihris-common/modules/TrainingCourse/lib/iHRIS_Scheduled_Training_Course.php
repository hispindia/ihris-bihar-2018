...
class iHRIS_Scheduled_Training_Course extends I2CE_List{
    ...
    public function enrolledStudentsList($node,$template,$readonly=false) {
        ...
        foreach ($exams as $examid=>$data) {
            ...
            foreach ($list as $id=>$sdata) {
                ...
                if ($has_exam && ($examNode =  $template->getElementByName('student_exam',0,$studentNode)) instanceof DOMElement) {                
                    $score = null;
                    $scores = I2CE_FormStorage::listFields('training_course_exam',array('score', 'retake'),$sdata['person_scheduled_training_course'],$where);
                    $exam_id ='0';
                    if ( count($scores) == 1) {
                    ...
                    if ($finalNode) {
                        ...
                        if( $readonly ){
                            $finalNode->setAttribute('readonly','readonly');
                        }
                    }
    ...
    public function enrolledStudentsListCompact($node,$template,$readonly=false) {
