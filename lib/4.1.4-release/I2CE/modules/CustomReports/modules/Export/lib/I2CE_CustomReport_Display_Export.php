    ...
    protected function processResultRowArray($row) {
        ...
        foreach ($this->cols as $key) {
            ...
            if (array_key_exists($key,$row)) {
                $vals[] = self::$col_begin[$this->style] . str_replace(self::$col_search[$this->style],self::$col_replace[$this->style],$row[$key]) . self::$col_end[$this->style]  ;
            } else {
                $vals[] = '';
            }
        }
...
";
    ...
    public function generateExport() {
        ...
        if (!($out = $this->processResults($data,$resultsNode=null))) {
        ...
        if ($style == 'xml' && $this->transform ) {
            ...
            if (!@$xslDoc->loadXML($this->defaultOptions['xslts'][$this->transform]['definition'])) {
    ...
    protected function getStyle() {
        ...
        if (!in_array($this->style,array('csv','tab','xml'))) {
            $this->style ='html';
        }
        if ($this->transform) {
    ...
    public function getFileName($style = null, $compression = null) {
        ...
        $filename = addslashes(str_replace(array(' ',"\n","\t") , array('_',' ','_'),$this->config->display_name)) . '_' . date("d_m_Y") . '.' . $style;
        if ($compression === null) {
    ...
    public function display($contentNode,$processResults=true, $controls = null) {        
        ...
        if (array_key_exists('HTTP_HOST',$_SERVER)) {
            ...
            case 'gz':
                ...
                break;
                //gzip is an encoding and not a mime-type so we fall thruogh here.
            default:
                switch ($style) {
                case 'tab':
