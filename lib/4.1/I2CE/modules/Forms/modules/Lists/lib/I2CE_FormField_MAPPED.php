...
abstract class I2CE_FormField_MAPPED extends I2CE_FormField_DB_STRING { 
    ...
    protected function _create_DOMEditable_ajax_list($node, $template, $form_node,$show_hidden = 0, $multiple=false) {
        ...
        foreach( array_keys( $form_limits ) as $limited_form ) {
            $max_disp = count($disp_fields);
            for( $i = 0; $i < $max_disp; $i++ ) {
                if ( $disp_fields[$i] == $limited_form ) {
        ...
        if ( $is_single ) {
            ...
            if ( is_array( $defaults ) && count( $defaults ) > 0 ) {
                ...
                if ( !end($ele_array) ) {
                    array_pop( $ele_array );  // Remove the last blank from this if it's a standard form element
                }
        ...
        foreach( $disp_fields as $disp_idx => $disp_field ) {
            ...
            $block_name = $ele_name . '_block_' . $disp_field;
            ...
            $selectOpts = array( 'name' => 'ajax_list_' . $ele_name . '[' . $disp_field . ']', 
                        'id' => $ele_name . '_' . $disp_field, 'disabled' => 'disabled' );
            if ( !$first ) {
            ...
            if ( $is_single ) {
                ...
                $js[] = "  $('${ele_name}_${disp_field}').addEvent('change', function() {\n"
                    . ($next_field != '' && ($two_columns?$disp_idx<count($disp_fields)-2:true) ? "    populateAjaxList( this, '${ele_name}', '${next_field}', '$ff', '" 
                            . $next_field_form . "', '" . urlencode($disp_field) . "', this.options[this.selectedIndex].value );\n"
                            : '')
