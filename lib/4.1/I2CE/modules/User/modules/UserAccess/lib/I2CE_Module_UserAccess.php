...
class I2CE_Module_UserAccess extends I2CE_Module {
    ...
    public function action_initialize() {
        ...
        $qrs[] = 'CREATE TABLE IF NOT EXISTS ' . $options['logTable'] . ' '
            ...
            .' `ip_address` varchar(32) default "127.0.0.1",'
            .' KEY `user` (`user`),'
            .' KEY `login` (`login`)'
    ...
    public function upgrade($old_vers,$new_vers) {
        ...
        if (I2CE_Validate::checkVersion($old_vers,'<','4.1.9.1')) {
            if (! $this->updateUserLogTable()) {
                return false;
            }
        }
        
        if (I2CE_Validate::checkVersion($old_vers,'<','4.1.9.3')) {
    ...
    protected function updateUserLogTable(){
        $db = MDB2::singleton();
        $qry_show = "SHOW COLUMNS FROM user_log LIKE 'ip_address'"; 
        $qry_alter = "ALTER TABLE user_log ADD COLUMN `ip_address` varchar(32) default '127.0.0.1'"; 
        $results = $db->query( $qry_show );
        if ( I2CE::pearError( $results, "Error getting columns from user_log table: on {$qry_show}" ) ) {
            return false;
        }
        if( $results->numRows() != 1) { //the logic is vague: I could not find another logic to confirm the field is found
          ...
          if ( I2CE::pearError( $qry_alter_results, "Error getting columns from user_log table: on {$qry_alter}" ) ) {
            return false;
          }
        }
        return true;
