...
class iHRIS_PageFormParentPerson extends I2CE_PageForm {
    ...
    protected function action() {
        if ( $this->get_exists('delete') && $this->get('delete') == 1 ) {
            I2CE::raiseMessage(print_r($this->args,true));
            $can_mass_delete = false;
            if ( array_key_exists( 'admin_mass_delete', $this->args ) && $this->args['admin_mass_delete'] 
                    && $this->user->username == 'i2ce_admin' ) {
                $can_mass_delete = true;
            }
            $parent_id = $this->getPrimary()->getParent();
            $form_id = $this->getPrimary()->getNameId();
            $child_forms = $this->getPrimary()->getChildForms();
            if ( count($child_forms) > 0 && !$can_mass_delete ) {
                ...
                if ( $allowable ) {
                    if ( $this->checkActionPermission('delete') ) {
                        if ( count($child_forms) > 0 && $can_mass_delete ) {
                            $storage = $this->getPrimary()->getStorage();
                            if ( $storage == 'entry' ) {
                                $deleted = I2CE_FormStorage_entry::massDelete( array( $this->getPrimary()->getId() ), array(), false );
                                if ( $deleted !== false ) {
                                    $message = "The requested form and children have been deleted ($deleted forms).";
                                } else {
                                ...
                                I2CE::raiseMessage("Tried to delete a child form $form_id when there are possible child forms and storage isn't entry for i2ce_admin.");
                                $message = "This form can not be deleted.";
                                I2CE::getConfig()->setIfIsSet( $message, "/modules/forms/page_feedback_messages/person_child_delete_not_allowed" );
                            }
                        } else {
