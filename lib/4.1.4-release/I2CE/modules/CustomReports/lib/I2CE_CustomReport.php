...
class I2CE_CustomReport extends I2CE_Fuzzy{
    ...
    public function generateCache($force=false,$cache_forms = true) {
        ...
        if ($cache_forms) {
            ...
            if (count($failures) > 0) {
                ...
                I2CE::raiseError("Successfully cached all forms");
            }
        }
    ...
    protected function setupQueries() {
        ...
        $keys = array("last_md5 BINARY(16) UNIQUE","md5 BINARY(16) UNIQUE",'KEY (`primary_form+id`)'  );                    
        ...
        foreach ($formNames as $formName) {
            ...
            $reporting_fields =    $this->rel->getLimitingFields($formName); //get the fields used in limits.            
            ...
            if ($prev_tmp_table) {
                ...
                if ( stripos( $join, 'SELECT' ) !== false 
                        || $join_style == 'RIGHT JOIN' || !$limit_one) {
        ...
        foreach ($reportingFunctions as $function=>$data) {
            ...
            if ($data['aggregate']) {
                ...
                if (count($data['aggregate']) > 0) {
                    //$groupby = ' GROUP BY `' . implode("`,`" ,$data['aggregate']) . '`';
                    $groupby = ' GROUP BY `' . implode("`,`" ,array_diff($c_fields,array_merge($data['aggregate'],$data['required_fields']))) . '`';
                    if (!$agged) {
            ...
            if ( $groupby != '' ) {
                ...
                $this->populate_queries[$counter]['update'] =
                    "UPDATE {$curr_tmp_table} SET `+$function` = "
                    . $data['qry'];
            }
