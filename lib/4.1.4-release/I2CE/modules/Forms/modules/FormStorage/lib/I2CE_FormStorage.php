...
class I2CE_FormStorage extends I2CE_Module {
    ...
    public static function moveData($form, $source,$target,$user, $ids = null) {
        ...
        foreach ($ids as $id) {
            ...
            if (!self::_save($formObj,$user,false,$target)) {
                I2CE::raiseError("Could not save $form|$id to " . get_class($source));
                $formObj->cleanup();
                $success =false;
                continue;
            }
    ...
    public static function exportData($form,$target, $user, $ids = null) {
        $source = self::getStorage( $form_name);            
    ...
    public static function getMechanismByStorage( $storage ) {
        ...
        if (!array_key_exists($storage, self::$storageMechs) || !self::$storageMechs[$storage] instanceof I2CE_FormStorage_Mechanism) {
            ...
            if (!class_exists($mechanism) || !is_subclass_of($mechanism,'I2CE_FormStorage_Mechanism')) {
                return false;
            }
            $options = array();
            ...
            foreach( $field_node as $id => $fields ) {
                ...
                foreach( $field_list as $field => $map_form ) {
                    ...
                    if ( is_array( $new_value ) && array_key_exists( 'new_value', $new_value ) 
                        ...
                        $fieldObj->setFromDB( $new_value['new_value'] );
                        $fieldObj->setAttribute( "DBEntry_change_type", I2CE_FormStorage_Mechanism::CHANGE_UPDATE );
     ...
     */
    public function populateChild( $form, $form_name, $order_by = null, $where = null, $type = 'default' ) {
        if ( !$form_name) {
        ...
        if (I2CE::getConfig()->is_parent($child_data_path)) {
            ...
            if ($child_data->is_parent('limits')) {
                $where = $child_data->limits->getAsArray();
            }
            if ($child_data->is_scalar('order')) {
                $order_by = explode(',',$child_data->order);
            }
        }        
        $ids = $form->getChildIds( $form_name, $order_by, $where );
        foreach( $ids as $id ) {
    ...
    public function storeHistory($form) {
        ...
        $db = MDB2::singleton();
        if (!$this->store_stmt) {
            $this->store_stmt = $db->prepare("INSERT INTO form_history (formid,history,date) VALUES (?,?,NOW())"   , array( 'text', 'blob' ) ,MDB2_PREPARE_MANIP);
     ...
     * @return array
     */
    public function getRecords( $factory, $form_name) {
        ...
        $ret =  $storageMechanism->getRecords($form_name);
        if (!is_array($ret)) {
