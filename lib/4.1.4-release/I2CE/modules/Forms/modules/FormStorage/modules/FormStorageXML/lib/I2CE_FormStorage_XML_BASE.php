...
abstract class I2CE_FormStorage_XML_BASE extends   I2CE_FormStorage_File_Base{
     ...
     *                 'xpath' => DOMXPath, 
     *                 'dataset' => DOMNode,
     *                 'namespace' = string //the name space we are working with
     *                 )
     */
    ...
    protected function getDOMData( $form ) {
        ...
        if ( $entries->length != 1 ) {
            I2CE::raiseError( "Could not find " . $this->getBaseQuery($form) . " in $file_loc." );
            return false;
        }
    ...
    protected function getFieldDataDesc($form,$field = null) {
        ...
        if  ($options->is_parent('fields')) {
            foreach ($options->fields as $field=>$fieldConfig  ) {
                if (
                    !$fieldConfig instanceof I2CE_MagicDataNode
                    || !$fieldConfig->is_scalar('attribute') 
                    || !$fieldConfig->attribute) {
                    continue;
                }
                $this->fieldData[$form][$field] = $fieldConfig->getAsArray();
            }
        }
        foreach ($this->fieldData[$form] as $field => &$fieldData){
