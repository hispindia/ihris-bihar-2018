   ...
   /**
    ...
    public function setDefaultOptions($fieldData) {
        ...
        if (!is_array($ret) || count($ret) != 2) {
            ...
            list($value_set,$default_value) = $ret;
            if ($value_set) {
    ...
    public function processDOM($node,$template) {
        ...
        if ( $node->hasAttribute( "href") ) {
            ...
            if ( $node->hasAttribute( "class" ) ) {
                $form_node->setAttribute( "class", $node->getAttribute("class") );
            }
            $title_attr = null;
            ...
            } elseif ( $template->isReview() || ( $node->hasAttribute( "noedit" ) && $this->issetValue() ) ) { 
                if ($field_node) {
