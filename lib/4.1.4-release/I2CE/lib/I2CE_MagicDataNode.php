...
//             }            
                        ...
                        if ($this->is_translated($locale)) {  //I2CE_Locales::DEFAULT_LOCALE always returns translated
                            ...
                            if (!$trans) {
                                if ($locale == I2CE_Locales::DEFAULT_LOCALE) {
                                    //do nothing.  it may have not really been set.
                                } else {
                                    return $trans;
                                }
                            } else {
        ...
        public function setValue( $value, $locale = null, $set_default_locale = false) {
            ...
            if ($value instanceof I2CE_MagicDataNode) {
                if (!  $value->populate()) {
                    I2CE::raiseError("Could not populate at " . $value->getPath());
                    return false;
                }
                //now copy over any attributes
                foreach ($value->attributes as $prefix=>$attributes) {
                    foreach ($attributes as $k=>$attr) {
                        $value->_setAttribute($k,$attr->value,$prefix);
                    }
                }
                if ($value->type == self::TYPE_INDETERMINATE) {
                    //do nothing                    
                    return true;
                }
            }
         ...
         * @param boolean $recurse.  Defaults to false.  If true, and we were able to succesfully save this node, we recrusively save all the populated children
        ...
        protected function _removeAttribute($prefix,$key) {
            ...
            if (!$this->array_key_exists($key,$this->attributes[$prefix])) {
                return false;
            }
