...
$tmp_dir_base = "/home/" . trim(`whoami`) . '/ihris-release';
...
foreach ($t_dirs as $i=>$dir) {
    ...
    if (count($out)>0) {
        if (!prompt("Ignore unomitted changes ($dir)?",$uncommitted)) {
...
foreach ($top_dirs as $dir) {
    ...
    $cmd = "grep  -P 'I2CEConfiguration\s+name' $dir/*xml ";
    ...
    if ($last_release_comps[0] != $vers_comps[0]) {
...
$cmd ="rm -fr $tmp_dir";
if (is_dir($tmp_dir) &&  simple_prompt("The direcotry $tmp_dir already exists.  Remove it [$cmd]?"))  {
    exec($cmd);   
}
`mkdir -p $tmp_dir`;
...
if (simple_prompt("Check for changes since last release?")) {
...
if (simple_prompt("Create Release Branches?")) {
    ...
    foreach ($top_mod_dirs as $mod=>$dir) {
        $next_release = $last_release[$mod]['next_release'];
        $lp_release = "lp:~{$launchpad_login}/" . strtolower($mod) . "/$next_release-release";
...
if ($ask_upload = simple_prompt("Create tarballs?")) {
    ...
    foreach ($top_mod_dirs as $mod=>$dir) {
        ...
        $cmd ="cd $tmp_dir && tar -cj --exclude-backups --exclude-vcs --exclude=.bzr*  --exclude=packaging/* -f $tar_ball $mod";
    ...
    I2CE::raiseError("Creating $tar_ball");
    $cmd ="cd $tmp_dir && tar -cj  --exclude-backups --exclude-vcs --exclude=.bzr*  --exclude=packaging/*  -f $tar_ball " . implode(" ", array_keys($top_mod_dirs));
    exec ($cmd);
}
...
if ($ask_upload && simple_prompt("Upload tarballs to launchpad?")) {
    ...
    foreach ($created_tarballs as $top_mod=>$tar_balls) {
        ...
        $version = $last_release[$top_mod]['next_release'];
