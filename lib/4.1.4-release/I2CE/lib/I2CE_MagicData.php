...
class I2CE_MagicData extends I2CE_MagicDataNode {
    ...
    public function store( $node, $max = -1 ) {
        ...
        for( $i = $max; $i >= 0; $i-- ) {
            $success = $success && $this->storage[$i]->store( $node );
            if (!$success) {
                I2CE::raiseError( "Failed to store for $i: " . get_class( $this->storage[$i] ) );
                $this->storage[$i]->destroy( $node );
            }
        }
        if (!$success )  {
            I2CE::raiseError("Could not store for " . $node->getPath(false));
        }
        return $success;
    }
    /**
