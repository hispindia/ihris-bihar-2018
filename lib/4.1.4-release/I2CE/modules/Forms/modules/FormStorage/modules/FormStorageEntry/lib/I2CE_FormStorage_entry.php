...
class I2CE_FormStorage_entry extends I2CE_FormStorage_DB {
    ...
    protected function _getRequiredFieldsQuery($form,$fields, $id = null, $parent = false, $field_reference_callback = null, $mod_time = -1, $last_entry = true, $user=false) {
        ...
        if ($form_id == 0) {
            //I2CE::raiseError("No form/unable to create $form");
            return false;
        }
        ...
        //if (true) {
            ...
            if (!is_bool($parent) && is_scalar($parent)) {
                ...
                $wheres[] = "(r.parent_form = '". mysql_escape_string($parent_form) . "' AND r.parent_id = '" . mysql_escape_string($parent_id) . "')";
        ...
        $qry = ' SELECT '.  implode( ", ", $select_list ) . " FROM "  . implode( " LEFT JOIN ", $joins ) ;
        if (is_scalar($id)) {
    ...
    protected function getFormId( $form, $nocreate = false ) {
        ...
        $row = $this->db->getRow( "SELECT id FROM form WHERE name = ?", 
                                  array('integer'), array( $form ), array('text') );
        if ( isset( $row ) ) {
            ...
            if ( !I2CE::pearError( $form_id, "Error getting form id" ) ) {
                ...
                $res = $this->db->autoExecute( 
                    "form", $field_values, MDB2_AUTOQUERY_INSERT, 
                    null, array('integer', 'text' ) );
    ...
    protected function getFormFieldIdAndType( $form_name, $field_name ) {
        ...
        if ( isset( $row ) && !I2CE::pearError( $row, "Error getting field id and type:" ) ) {
            $data = array( 'id' => $row->id, 'type' => $row->type );
        ...
        foreach( $form as $key => $field ) {
            ...
            if ( $field->isInDB() ) {
                ...
                if ( array_key_exists( $field_id, $form_fields ) ) {
                    ...
                    $deleted = self::$prepared['setupForm']['delete']->execute( 
                            array( $form_id, $field->getName() ) );
                    ...
                    $res = $this->db->autoExecute( "form_field", $field_values, MDB2_AUTOQUERY_INSERT, null, 
    ...
    protected function prepareRecordStatement( $type ) {
        ...
        if ( !isset( self::$prepared['record'][$type] ) ) {
            if ( $type == "insert" ) {
                self::$prepared['record'][$type] = $this->db->prepare( "INSERT INTO record ( id, last_modified, form, parent_form, parent_id ) VALUES ( ?, NOW(), ?, ?, ? )",  
                        array( 'integer', 'integer', 'text', 'text' ),
                        MDB2_PREPARE_MANIP );
            } elseif ( $type == "update" ) {
                self::$prepared['record'][$type] = $this->db->prepare( "UPDATE record SET parent_form = ?, parent_id = ?, last_modified = NOW() WHERE id = ?", 
    ...
    public function save( $form, $user,$transact) {
        ...
        if ( $form->getId() == '0' ) {
            ...
            $res = self::$prepared['record']['insert']->execute( 
                array( $new_id, $form->getAttribute( "DBEntry_form_id" ), 
        ...
        foreach( $form as $key => $field ) {
            ...
            $field->setAttribute( "DBEntry_change_type", $change_type );
    ...
    public function delete( $form, $transact) {
        ...
        $delete = $this->db->prepare( "DELETE FROM record WHERE id = ?", array( 'integer' ), MDB2_PREPARE_MANIP );
        if ( I2CE::pearError( $delete, "Invalid delete form statement from record:" ) ) {
            if ( $transact && $this->db->in_transaction ) {
                $this->db->rollback();
            }
            return false;
        }
        if ( I2CE::pearError( $delete->execute( $form->getId() ), "Delete failed from record for " . $form->getId() ) ) {
            if ( $transact && $this->db->in_transaction ) {
                $this->db->rollback();
            }
            return false;
        ...
        foreach( array( 'entry', 'last_entry' ) as $table ) {
            $delete = $this->db->prepare( "DELETE FROM $table WHERE record = ?", 
                                          array( 'integer' ), MDB2_PREPARE_MANIP );
            if ( I2CE::pearError( $delete, "Invalid delete form statement from $table:" ) ) {
                if ( $transact && $this->db->in_transaction ) {
                    $this->db->rollback();
                }
                return false;
            }
            if ( I2CE::pearError( $delete->execute( $form->getId() ), "Delete failed from $table for " . $form->getId() ) ) {
                if ( $transact && $this->db->in_transaction ) {
                    $this->db->rollback();
                }
                return false;
    ...
    protected function FF_isSameValue( $form_field, $do_check) {
        if ( $do_check ) {
            ...
            $res = self::$prepared['check'][$form_field->getTypeString()]->execute( 
                    array( $form_field->getContainer()->getId(), $form_field->getAttribute( "DBEntry_form_field_id" ) ) );
    ...
    public function FF_save( $form_field, $do_check, $user ) {
        ...
        $result = self::$prepared['save'][$form_field->getDBType()][$form_field->getTypeString()]->execute( 
            array( $form_field->getContainer()->getId(),  //record 
                   $form_field->getAttribute( "DBEntry_form_field_id" ),  //form_field 
                   $user->getId(), //who 
                   $form_field->getAttribute( "DBEntry_change_type" ),  //change_type 
    ...
    public function FF_populateHistory( $form_field) {
        ...
        $result = $sth->execute( array( $form_field->getContainer()->getId(),
                                        $form_field->getAttribute( "DBEntry_form_field_id" ) ) );
