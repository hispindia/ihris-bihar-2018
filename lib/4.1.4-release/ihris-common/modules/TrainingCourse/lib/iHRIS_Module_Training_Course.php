...
class iHRIS_Module_Training_Course extends I2CE_Module{
    ...
    public function action_person_scheduled_training_course($page) {
        ...
        $person->populateChildren('person_scheduled_training_course');
        $pstcs = $person->getChildren('person_scheduled_training_course');
        if (count($pstcs) == 0) {
            return true;
        }
            ...
            if ($do_exam) {
                ...
                if (count($exams) > 0) {
                    ...
                    foreach ($exams as $exam) {
                        if (!  ($examNode = $template->appendFileById('view_training_course_exam.html','tbody','exam_results')) instanceof DOMNode) {
                            continue;
                        }
    ...
    protected function moveScheduledTrainingCourseToMap() {
        ...
        foreach ($stcs as $stc_id=>$data) {
            ...
            if (empty($parent) || $parent == '|'  || $parent == 'training_course|' || $parent =='training_course' 
                ...
                var_dump(array(empty($parent) ,
                               $parent == '|'  , 
                               $parent == 'training_course|' , 
                               $parent =='training_course' , 
                               !is_string($parent) , 
                               substr($parent,0,16) != 'training_course|' 
                             ));
                continue;
            }
    ...
    public function showEnrolledCourses($node,$template,$args=array()) {
        ...
        $trainingIds = $personForm->getChildIds('person_scheduled_training_course','-request_date'); //get all trainings.
